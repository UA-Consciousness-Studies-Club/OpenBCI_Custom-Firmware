# v3.3.0-vsc (VSC_OpenBCI_Custom-Firmware fork, 2026-06-03)

### New Features — extended commands for desktop-app versatility

All under one new top-level initiator `n` + a context-local sub-op + ASCII-hex args + `Z` latch.
Additive and gated; default streaming behaviour is unchanged. Aimed at multi-paradigm instruments
like MentalMouseLab. Full contract in `docs/WIRE_FORMAT.md` §2b.

- **Atomic acquisition profiles** (`n p` define / `n a` apply): store up to 4 RAM profile slots
  (active-channel mask + gain + input + bias + SRB2) and apply one to all 16 channels in a single
  `writeChannelSettings()` pass, resuming continuous read without a full stream stop/start. Lets a
  desktop app flip per-paradigm front-ends (occipital high-gain for steady-state, sensorimotor for
  motor imagery, low-gain jaw channel for electromyography click) in one command instead of 16
  per-channel `x` round-trips. Sample rate is intentionally excluded from the atomic path (changing
  it restarts the ADS).
- **Atomic channel power mask** (`n c`): power on a masked set of channels and power down the rest
  (keeping each channel's gain/input) in one pass. True bandwidth subsetting is deliberately NOT
  done because it would break the fixed 33-byte packet that BrainFlow parses.
- **Continuous contact/railing telemetry** (`n t`): emit a status frame (aux magic `0x0FC6`) every
  N samples carrying a per-channel railing bitmask and a per-channel lead-off contact bitmask. Costs
  no extra SPI — lead-off comes from the ADS status word already read each sample, railing from the
  channel ints already in memory. Drives a live electrode-quality indicator during a session.
- **Sample-locked event codes** (`n e`): latch a 16-bit event code (aux magic `0x0FC7`) into the
  very next emitted sample — sample-accurate event onsets and a paradigm/trial/condition code that
  maps cleanly onto a Brain Imaging Data Structure `events.tsv`, independent of `BOARD_MODE_MARKER`.
- Added `getHexForAsciiChar` (shared with the impedance commands) and an `AcqProfile` struct.

### Notes

- Validated host-side:
  `tests-ptw-assert/host_logic/extended_commands_logic_test.cpp` (27 assertions, all passing).
- Three RAW_AUX marker magics are now in use and mutually exclusive per packet:
  `0x0FC5` fast-cycle impedance, `0x0FC6` telemetry, `0x0FC7` event code.

# v3.2.0-vsc-fastcycle (VSC_OpenBCI_Custom-Firmware fork, 2026-06-02)

### New Features

- **Fast-cycle impedance** (`k` command): the firmware sequences one channel's P-input
  lead-off at a time entirely on the PIC32 — drive ON, dwell N samples, OFF, advance —
  **without** the per-channel stream stop/restart the stock `z` path pays. Clean
  single-channel return path (no shared-bias cross-talk) at a fraction of the host-driven
  sweep latency. Command: `k` + 4 hex channel mask + 2 hex dwell-samples + `Z`.
- **All-at-once batched lead-off** (`g` command): sets the P-input lead-off on a 16-channel
  mask in one `SDATAC`/`RDATAC` pass (no per-channel stop/restart), backing the host-side
  parallel impedance style. Command: `g` + 4 hex channel mask + `Z`.
- **Fast-cycle stop** (`l` command): single char, immediately clears all lead-off and
  restores the previous packet type.
- **DC lead-off status surfacing** (`m` command): optionally rides the ADS1299
  `LOFF_STATP`/`LOFF_STATN` per-channel comparator bits in the fast-cycle aux flags for an
  instant connected/not signal. Command: `m` + (`1`|`0`) + `Z`.
- Per-channel **segmentation marker**: during a fast-cycle impedance sweep each sample is emitted as a
  `RAW_AUX` packet whose 6 aux bytes carry `[magic 0x0FC5][active channel][flags]`, so the
  host can segment the single streamed window per channel. See `docs/WIRE_FORMAT.md`.
- Reinstated the previously commented-out `leadOffSetForChannel`, `leadOffSetForAllChannels`,
  `leadOffConfigureSignalForAll`, and `leadOffConfigureSignalForTargetSS` implementations
  (they were declared in the header but had no body).

### Notes

- **The stock single-channel `z CH P N Z` impedance path is unchanged.** The default packet
  format is unchanged unless a fast-cycle impedance sweep is explicitly commanded. New command chars
  `g`/`k`/`l`/`m` were chosen to collide with neither stock firmware nor the SD char handler.
- Per-channel lead-off *frequencies* remain out of scope (the ADS1299 `FLEAD_OFF` field is a
  single global setting — silicon limit), as does removing shared-bias cross-talk (board
  analog topology). See `docs/IMPEDANCE_HARDWARE_FIRMWARE_DISCUSSION.md`.
- Parser + sequencer logic validated host-side:
  `tests-ptw-assert/host_logic/impedance_fastcycle_logic_test.cpp` (35 assertions).
