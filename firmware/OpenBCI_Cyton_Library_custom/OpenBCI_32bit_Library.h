...
public:
  ...
  typedef enum MULTI_CHAR_COMMAND {
    ...
    MULTI_CHAR_CMD_PROCESSING_INCOMING_SETTINGS_IMPEDANCE, // custom: g/k/m args (VSC_OpenBCI_Custom-Firmware)
    MULTI_CHAR_CMD_PROCESSING_INCOMING_SETTINGS_EXTENDED   // custom: n <subop> args (VSC_OpenBCI_Custom-Firmware)
  };

  // custom: one acquisition-profile slot (RAM-resident; the host is the source of truth)
  typedef struct {
    boolean  valid;
    uint16_t mask;      // active-channel bitmask (bit i -> channel i+1)
    byte     gainReg;   // ADS gain register value (gainCode << 4)
    byte     inputReg;  // ADSINPUT_* code
    boolean  bias;      // include active channels in bias generation
    boolean  srb2;      // connect active channels to SRB2
  } AcqProfile;
  ...
  byte    getHexForAsciiChar(char);   // custom: ASCII hex nibble (0-9,A-F,a-f) -> 0..15
  ...
  void    leadOffInlineSetChannel(byte channelNumber, byte pInput, byte nInput); // write LOFF without tearing down the stream
  void    leadOffBatchSetMask(uint16_t pMask);   // set P lead-off on a 16-ch mask in one SDATAC/RDATAC pass
  void    impedanceFastCycleStart(uint16_t pMask, byte dwellSamples);
  void    impedanceFastCycleStop(void);
  void    serviceImpedanceFastCycle(void);       // stepped once per emitted sample while active
  void    processIncomingImpedanceSettings(char);
  // --- custom extended commands: profiles / channel mask / telemetry / events ---
  void    processIncomingExtendedSettings(char);
  void    extProfileDefine(byte slot, uint16_t mask, byte gainReg, byte inputReg, boolean bias, boolean srb2);
  boolean extProfileApply(byte slot);            // atomic, stream-preserving; false if slot invalid
  void    extChannelPowerMask(uint16_t mask);    // atomic active-channel power mask
  void    buildContactTelemetryFrame(void);      // fills auxData with the 0x0FC6 status frame
  void    ledFlash(int);
  void    loop(void);
  void    printAll(char);
  ...
  boolean   impedanceFastCycleActive;   // gate: per-sample service only runs when true
  boolean   impedanceDcStatusActive;    // surface LOFF_STATP/STATN bits in the aux flags
  uint16_t  impedanceChannelMask;       // bitmask (bit i = channel i+1) of channels to sweep
  byte      impedanceActiveChannel;     // 1-based channel currently driven (0 = none / done)
  byte      impedanceActiveChannelIdx;  // 0-based walk position within the mask
  byte      impedanceDwellSamples;      // samples to dwell on each channel
  byte      impedanceDwellCounter;      // samples elapsed on the current channel
  char      impedanceMultiCharMode;     // which initiator char (g/k/m) is being parsed
  byte      impedanceArgCounter;        // bytes consumed by the impedance multi-char parser
  char      impedanceArgBuffer[8];      // raw arg chars for the impedance multi-char parser
  PACKET_TYPE impedancePrevPacketType;  // packet type to restore when the sweep ends

  // --- custom extended-command state (VSC_OpenBCI_Custom-Firmware) ---
  AcqProfile extProfiles[OPENBCI_NUMBER_OF_PROFILE_SLOTS]; // RAM-resident acquisition profiles
  byte      extArgCounter;              // bytes consumed by the extended multi-char parser
  char      extArgBuffer[12];           // raw arg chars (sub-op + args + latch)
  byte      extTelemetryCadence;        // emit a contact/railing frame every N samples (0 = off)
  boolean   extEventCodePending;        // an event code is queued for the next emitted sample
  uint16_t  extEventCode;               // the queued 16-bit event code
