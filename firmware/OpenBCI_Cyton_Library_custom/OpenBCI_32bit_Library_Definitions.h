 ...
 *  These are ADDITIVE. The stock single-channel 'z CH P N Z' path is unchanged.
 *  Args are ASCII hex (printable, debuggable) and terminate with the 'Z' latch.
 *  Command chars g/k/l/m are unused by stock firmware and by the SD char handler.
 */
#define OPENBCI_IMPEDANCE_ALL_AT_ONCE_SET 'g'   // g + 4 hex P-mask + 'Z'
#define OPENBCI_IMPEDANCE_FAST_CYCLE_SET  'k'   // k + 4 hex P-mask + 2 hex dwell + 'Z'
#define OPENBCI_IMPEDANCE_FAST_CYCLE_STOP 'l'   // single char, immediate stop
#define OPENBCI_IMPEDANCE_DC_STATUS_SET   'm'   // m + ('1'|'0') + 'Z'

// Number of bytes the parser consumes AFTER the initiator, including the 'Z' latch
#define OPENBCI_NUMBER_OF_BYTES_IMPEDANCE_ALL_AT_ONCE 5  // 4 hex + 'Z'
#define OPENBCI_NUMBER_OF_BYTES_IMPEDANCE_FAST_CYCLE  7  // 4 hex + 2 hex + 'Z'
#define OPENBCI_NUMBER_OF_BYTES_IMPEDANCE_DC_STATUS   2  // 1 char + 'Z'

// Segmentation marker that rides in auxData[0] during a fast-cycle impedance sweep.
// Host watches for this in the first aux short to know the packet is tagged.
#define OPENBCI_IMPEDANCE_MARKER_MAGIC 0x0FC5

// Fast-cycle dwell (samples per channel) defaults / bounds
#define OPENBCI_IMPEDANCE_FAST_CYCLE_DWELL_DEFAULT 32  // ~128 ms @ 250 SPS
#define OPENBCI_IMPEDANCE_FAST_CYCLE_DWELL_MIN     1
#define OPENBCI_IMPEDANCE_FAST_CYCLE_DWELL_MAX     255
// Samples at the start of each channel's dwell flagged "settling" (host discards)
#define OPENBCI_IMPEDANCE_FAST_CYCLE_SETTLE        8

// auxData[2] flag bits during a fast-cycle impedance sweep
#define OPENBCI_IMPEDANCE_FLAG_SETTLING    (0x0001)  // sample is in the settle window
#define OPENBCI_IMPEDANCE_FLAG_LAST_SAMPLE (0x0002)  // last sample of this channel's dwell
#define OPENBCI_IMPEDANCE_FLAG_DONE        (0x0004)  // sweep complete (channel field = 0)
#define OPENBCI_IMPEDANCE_FLAG_DC_STATP    (0x0100)  // LOFF_STATP bit for the active channel
#define OPENBCI_IMPEDANCE_FLAG_DC_STATN    (0x0200)  // LOFF_STATN bit for the active channel

/** Extended commands for desktop-app versatility (VSC_OpenBCI_Custom-Firmware).
 *  ONE top-level initiator 'n', then a context-local sub-op char + ASCII-hex args + 'Z'.
 *  Sub-op chars are only parsed after 'n', so they collide with nothing.
 *    n p <slot:1h><mask:4h><gain:1h><input:1h><flags:1h> Z   define acquisition profile slot
 *    n a <slot:1h> Z                                          apply profile slot (atomic)
 *    n c <mask:4h> Z                                          set active channel power mask (atomic)
 *    n t <cadence:2h> Z                                       contact/railing telemetry every N samples (0=off)
 *    n e <code:4h> Z                                          latch a 16-bit event code into the next sample
 *  gain code 0..6 -> x1/x2/x4/x6/x8/x12/x24 (register = code<<4).
 *  input code = ADSINPUT_* low nibble (0=normal,1=shorted,5=testsig,...).
 *  flags: bit0 = include in bias, bit1 = connect SRB2.
 */
#define OPENBCI_EXTENDED_CMD_SET 'n'
#define OPENBCI_EXT_SUBOP_PROFILE_DEFINE   'p'
#define OPENBCI_EXT_SUBOP_PROFILE_APPLY    'a'
#define OPENBCI_EXT_SUBOP_CHANNEL_MASK     'c'
#define OPENBCI_EXT_SUBOP_TELEMETRY        't'
#define OPENBCI_EXT_SUBOP_EVENT_CODE       'e'

// Total bytes the extended parser consumes per sub-op (incl. sub-op char and 'Z' latch)
#define OPENBCI_EXT_LEN_PROFILE_DEFINE 10  // p + slot + 4 mask + gain + input + flags + Z
#define OPENBCI_EXT_LEN_PROFILE_APPLY  3   // a + slot + Z
#define OPENBCI_EXT_LEN_CHANNEL_MASK   6   // c + 4 mask + Z
#define OPENBCI_EXT_LEN_TELEMETRY      4   // t + 2 cadence + Z
#define OPENBCI_EXT_LEN_EVENT_CODE     6   // e + 4 code + Z

#define OPENBCI_NUMBER_OF_PROFILE_SLOTS 4

// Aux-field marker magics (auxData[0]) for the custom RAW_AUX side channels
//   0x0FC5 = fast-cycle impedance marker (see impedance section above)
#define OPENBCI_TELEMETRY_MARKER_MAGIC 0x0FC6  // contact/railing telemetry frame
#define OPENBCI_EVENT_MARKER_MAGIC     0x0FC7  // sample-locked event code frame

// A channel is flagged "railing" when |sample| exceeds this (~99.2% of 2^23 full scale)
#define OPENBCI_RAILING_THRESHOLD 8323072L  // 0x7F0000
